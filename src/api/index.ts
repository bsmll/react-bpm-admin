...
export const processApi = {
  ...
  updateDefinition: (
    id: string,
    data: { name?: string; description?: string; definition?: any },
...
export const instanceApi = {
  ...
  getInstance: (id: string) => request.get(`/process-instances/${id}`),
...
export const taskApi = {
  ...
  getMyPendingTasks: () => request.get("/tasks/my-pending"),
...
/** 后端已在拦截器中解包为 data；此处返回表单字段数组 */
export const generateAiForm = (prompt: string) =>
  request.post<AiFormItemSchema[]>("/ai/generate-form", { prompt }, {
    timeout: 130_000,
